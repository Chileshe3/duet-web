...
import {
  WOULD_YOU_RATHER_PAIRS, watchActiveSessionId, startSession,
  watchSession, submitChoice, submitPrediction, advanceRound, endSession
} from "./wouldyourather.js";

const overlayEl = document.getElementById("wouldYouRatherOverlay");
const fabContainerEl = document.getElementById("wouldYouRatherFabContainer");
...
export function showGameFab(coupleId) {
  ...
  fabContainerEl.innerHTML = `<button id="wyrFab" class="game-fab game-fab-wyr">🤔</button>`;
  document.getElementById("wyrFab").onclick = () => openGameOverlay(coupleId);
}
...
function pairFor(pairId) {
  return WOULD_YOU_RATHER_PAIRS.find((p) => p.id === pairId);
}
...
function renderOverlay() {
  ...
  if (state.stage === "start") {
    bodyHtml = `
      ...
      <p>Eight dilemmas — most are quick picks, a couple are "guess what they'll choose." Lock in first, see who knows who best.</p>
      <button id="wyrStart">Start</button>
    `;
    ...
    bodyHtml = `
      ...
      <p class="game-prompt">Would you rather…</p>
      ...
      <div class="game-reveal">
        ${
          bothAnswered
            ? `<p class="game-result">${myChoice === partnerChoice ? "🎉 You matched!" : "Different picks this time"}</p><button id="wyrNext">Next</button>`
    ...
    bodyHtml = `
      ...
      <div class="row">
        <button id="wyrPlayAgain">Play again</button>
        <button id="wyrDone">Done</button>
      </div>
    `;
  }
  ...
  overlayEl.innerHTML = `
    <div class="game-card">
      <div class="game-header">
        ...
        <button id="wyrClose" class="icon-btn">×</button>
      </div>
      <div class="game-body">${bodyHtml}</div>
    </div>
  `;

  document.getElementById("wyrClose").onclick = () => {
    endSession(currentCoupleId).catch((e) => console.error("endSession failed", e));
    closeGameOverlay();
  };

  if (state.stage === "start") {
    document.getElementById("wyrStart").onclick = () => {
    ...
    if (state.bothAnswered) {
      document.getElementById("wyrNext").onclick = () => {
    ...
    document.getElementById("wyrPlayAgain").onclick = () => {
      startSession(currentCoupleId).catch((e) => console.error("startSession failed", e));
    };
    document.getElementById("wyrDone").onclick = () => {
      endSession(currentCoupleId).catch((e) => console.error("endSession failed", e));
      closeGameOverlay();
    };
  }
